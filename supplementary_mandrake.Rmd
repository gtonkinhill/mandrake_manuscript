...
```{r, eval=FALSE}
cl <- hdbscan(gps_embedding, minPts = 10)
clust <- cl$cluster
clust[clust==0] <- max(clust) + 1:sum(clust==0)
rand_mandrake <- fossil::rand.index(clust, ppunk_clsts)
...
clust[clust==0] <- max(clust) + 1:sum(clust==0)
rand_pca <- fossil::rand.index(cl_pca$cluster, ppunk_clsts)

cl_tsne <- hdbscan(tsne_embedding$Y[match(gpsnames$V1, names),], minPts = 10)
clust <- cl_tsne$cluster
clust[clust==0] <- max(clust) + 1:sum(clust==0)
rand_tsne <- fossil::rand.index(clust, ppunk_clsts)
...
clust[clust==0] <- max(clust) + 1:sum(clust==0)
rand_umap <- fossil::rand.index(clust, ppunk_clsts)
